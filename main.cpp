...
public:
  ...
  const string encode(const vector<string> &strs) {
    string encoded_str{};
  ...
  const vector<string> decode(const string s) {
...
int main() {
  ...
  Codec codec{};
  const string encoded_message{codec.encode(msg)};
  cout << "encoded message: " << encoded_message << endl;
  const vector<string> decoded_message{codec.decode(encoded_message)};
